    ...
    **kwargs,
    ...
    if hasattr(response, "__aiter__"):
        ...
        if content is None:
            ...
            logger.warning(f"Response finish_reason: {response.choices[0].finish_reason if response.choices else 'No choices'}")
            logger.warning(f"Prompt that caused empty response: {prompt[:200]}...")  # First 200 chars
...
async def qwen2_dot_5_14B_complete(
    prompt, system_prompt=None, history_messages=[], keyword_extraction=False, **kwargs
) -> str:
    keyword_extraction=kwargs.pop("keyword_extraction", None)
    if keyword_extraction:
